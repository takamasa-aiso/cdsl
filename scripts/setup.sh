#!/usr/bin/env bash
# Configure CDSL; sourcing also activates it in the calling Bash shell.

if [ -z "${BASH_VERSION:-}" ]; then
    printf '%s\n' 'CDSL: Run with Bash: bash ./scripts/setup.sh' >&2
    return 1 2>/dev/null || exit 1
fi
if declare -F _cdsl_setup_main >/dev/null; then
    printf '%s\n' 'CDSL: The shell function _cdsl_setup_main already exists.' >&2
    return 1 2>/dev/null || exit 1
fi

_cdsl_setup_main() {
    local cdsl_dry_run=0 cdsl_python_option='' cdsl_real_codex=''
    local cdsl_source=${BASH_SOURCE[0]} cdsl_root cdsl_kind cdsl_sourced=0
    local cdsl_activation='show Bash activation instructions'
    if [[ ${BASH_SOURCE[0]} != "$0" ]]; then
        cdsl_sourced=1
        cdsl_activation='activate this Bash shell'
    fi
    local -a cdsl_args=()
    while (( $# )); do
        case $1 in
            --help|-h)
                printf '%s\n' \
                    'Usage: bash ./scripts/setup.sh [--dry-run] [--python PATH] [--real-codex PATH]' \
                    'Installs missing dependencies with apt-get/dnf and configures CDSL.' \
                    'Use source ./scripts/setup.sh instead to activate CDSL in the current Bash shell.' \
    ...
    if (( EUID == 0 )) && [[ -n ${SUDO_USER-} ]]; then
        printf '%s\n' 'CDSL: Run this script as your normal user, without sudo. Only package commands use sudo.' >&2
        return 1
    fi
    ...
    cdsl_root=$(CDPATH= cd -- "${cdsl_source%/*}/.." && pwd -P) || return 1
    if [[ ! -f $cdsl_root/scripts/cdsl.py || ! -f $cdsl_root/cdsl/auto_setup.py ]]; then
        printf '%s\n' 'CDSL: Keep scripts/setup.sh in the complete cloned or extracted CDSL directory.' >&2
        return 1
    fi
...
PY
        ...
        if (( cdsl_dry_run && ${#cdsl_packages[@]} )); then
            printf 'Then: recheck dependencies, validate CDSL settings, install Codex 0.153.4 if missing, configure CDSL, and %s.\n' "$cdsl_activation"
            return 0
        fi
        hash -r
        cdsl_python=$(_cdsl_python) || cdsl_python=''
        ...
        if (( cdsl_probe_status == 1 )); then
            ...
            if (( cdsl_dry_run )); then
                printf '%s\n' 'Would download https://chatgpt.com/codex/install.sh and install Codex 0.153.4 as the current user.'
                printf 'Then: validate and install CDSL and %s.\n' "$cdsl_activation"
                return 0
            fi
        ...
        if (( cdsl_dry_run )); then
            ...
            # Print activation instructions below according to how this script was invoked.
        ...
        if (( cdsl_sourced )); then
            ...
            printf '%s\n' 'CDSL is ready in this Bash terminal. Run: codex'
        else
            printf '%s\n' 'CDSL is configured. Open a new Bash terminal, or run this in your current Bash shell:'
            printf '  source %q\n' "$HOME/.config/cdsl/shell.sh"
            printf '%s\n' 'Then run: codex'
        fi
        printf '%s\n' 'Complete Codex sign-in on first launch if prompted.'
        return 0
    else
        printf '%s\n' 'CDSL: Installation did not complete; this shell was not activated. Any packages or official Codex already installed are retained. Resolve the error and run scripts/setup.sh again.' >&2
        return 1
    fi
}

# Prevent setup commands from consuming a piped bootstrap's remaining input.
# The Python probe supplies its own heredoc and Codex receives /dev/null.
if _cdsl_setup_main "$@" </dev/null; then
    unset -f _cdsl_setup_main
