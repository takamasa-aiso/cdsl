...
- [表示と操作](#表示と操作)：各行の意味・Permissions・スクロール・コピーと貼り付け
...
- WSLでは、上段の文字をドラッグして選ぶとWindowsのクリップボードへコピーします。`Ctrl+v`で文字または画像をCodexの入力欄へ貼り付けられます。右クリックの動作は端末の設定に従います。CDSLは右クリック時に追加の貼り付けを行いません。
- 端末本来の文字選択は、対応する端末で`Shift`を押しながらドラッグします。WSL以外では、端末のコピー・貼り付け操作を使ってください。tmuxからのコピーには端末側のOSC 52対応が必要です。
...
WSLのコピー・貼り付け補助にはWindows PowerShellを使います。コピー対象の文字列をWindowsクリップボードへ送り、`Ctrl+v`では画像があればPNGへ変換し、なければ文字列を入力します。取得できなければCodex標準処理へ戻ります。
